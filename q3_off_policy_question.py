...
def main():
    ...
    while True:
        ...
        if not last_state == None:
            ...
            delta_etd = etd_algo.update(feature_vector(state),reward,feature_vector(state_prime),alpha,gamma(state),gamma(state_prime),lam,rho,1)
        # plot and save data
        d1.update(t, [ang * 10, theta[state_prime],etd_algo.get_value(feature_vector(state_prime)),gamma(state_prime) * 3, cummlant(state_prime) *6])
        d2.update(t,[delta,delta_etd])
        servo1_data.append([t, ang * 10, theta[state_prime],etd_algo.get_value(feature_vector(state_prime)), gamma(state_prime) * 3, cummlant(state_prime) *6,delta,delta_etd])
